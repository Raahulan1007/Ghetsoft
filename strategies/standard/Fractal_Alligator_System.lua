...
function ExtUpdate(id, source, period)
    if (id == 1) then
        ...
        local down, dperiod;
        down, dperiod = FindDnFractal(gBidSource:size()-1);
        if down > 0 then
            if (down < bALLIGATOR.Teeth[dperiod] and down < instance.bid[NOW]) then
                if not haveTrades('S') then
                    ...
                    if ShowAlert and prevDown ~= down then
                        local msg = string.format(resources:get("R_SetSellLevelAt"), tostring(down) .. "," .. core.formatDate(gAskSource:date(dperiod)));
    ...
    if (id == 2) then
        ...
        if (aALLIGATOR.Teeth:hasData(period)) then
            ...
            local up, uperiod;
            up, uperiod = FindUpFractal(gAskSource:size()-1);
            if up > 0 then
                if (up > aALLIGATOR.Teeth[uperiod] and up > instance.ask[NOW]) then
                    if not haveTrades('B') then
                        ...
                        if ShowAlert and up ~= prevUp then
                            local msg = string.format(resources:get("R_SetBuyLevelAt"), tostring(up) .. "," .. core.formatDate(gAskSource:date(uperiod)));
                            ExtSignal(source, period, msg, SoundFile, Email, RecurrentSound)
                            prevUp = up
                        end
                    end
...
function FindUpFractal(period)
    local i = period;
    local src = gAskSource.high;
    local first = src:first();
    local curr;
 
    while i > first + 5 do
        curr = src[i - 3];
        if src[i - 5] < curr and src[i - 4] < curr and src[i - 2] < curr and src[i - 1] < curr then
            return curr, i - 3;
        end
        i = i - 1;
    end
...
function FindDnFractal(period)
    local i = period;
    local src = gAskSource.low;
    local first = src:first();
    local curr;
