...
function Init()
    ...
    strategy.parameters:addString("TRAILING_STOP_TYPE", resources:get("R_TRAILING_STOP_TYPE"), resources:get("R_TRAILING_STOP_TYPE_D"), "Dynamic");
    strategy.parameters:addStringAlternative("TRAILING_STOP_TYPE", resources:get("R_DYNAMIC"), "", "Dynamic");
    strategy.parameters:addStringAlternative("TRAILING_STOP_TYPE", resources:get("R_FIXED"), "", "Fixed");
    strategy.parameters:addInteger("FIXED_TRAILING_STOP", resources:get("R_FIXED_TRAILING_STOP"), resources:get("R_FIXED_TRAILING_STOP_D"), 10, 10, 300);
...
function Update()
    if not(init) then
        ...
        if CANTRADE then
            ...
            if instance.parameters.TRAILING_STOP_TYPE == "Dynamic" then
                TRAILING_STOP_SIZE = 1;
            else
                TRAILING_STOP_SIZE = instance.parameters.FIXED_TRAILING_STOP;
            end
...
function enter(side)
    ...
    if STOP >= 1 then
        ...
        if USE_TRAILING_STOP then
            valuemap.TrailStepStop = TRAILING_STOP_SIZE;
        end
    end
    if LIMIT >= 1 then
