function Init()
    ...
    strategy.parameters:setFlag("Period", core.FLAG_BARPERIODS);
...
function Prepare()
    ...
    -- set the name 
    local name = profile:id() .. "(" .. instance.bid:instrument() .. "(" .. instance.parameters.Period  .. ")" .. "," .. instance.parameters.SN .."," ..
			                instance.parameters.LN .."," .. instance.parameters.IN ..")";
    instance:name(name);    	
    ...
    if instance.parameters.isNeedAutoTrading then
        ...
        local hedging = core.host:findTable("accounts"):find("AccountID", idAccount).Hedging;
        if hedging == "Y" then
            core.host:trace(resources:get("warning_no_hedging_support"));
        end
...
function Update()
    ...
    if size > 1 then
        ...
        if lastSerial == nil or serial ~= lastSerial then
            local period = size - 2;
