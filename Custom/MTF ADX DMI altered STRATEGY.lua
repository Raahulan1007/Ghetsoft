...
-- http://fxcodebase.com/code/viewtopic.php?f=31&t=23531
...
local FRAMES = {"H1", "H2", "H3"}
local TfNumber = 3

function Init() --The strategy profile initialization
    strategy:name("MTF ADX DMI altered STRATEGY")
...
--~     TIMEFRAME(1, FRAMES[1], 25)
--~     TIMEFRAME(2, FRAMES[2], 20)
--~     TIMEFRAME(3, FRAMES[3], 20)

    CreateTradingParameters()
end

function TIMEFRAME(id, FRAME, Level)
    strategy.parameters:addGroup(id .. ". Time Frame")

    if id > 1 then
        strategy.parameters:addBoolean("ON" .. id, "Use This Time Frame", "", true)
    end

    strategy.parameters:addString("TF" .. id, "Fast MA time frame", "", FRAME)
    strategy.parameters:setFlag("TF" .. id, core.FLAG_PERIODS)

--~     strategy.parameters:addInteger("AP" .. id, "ADX Period", "", 14)
--~     strategy.parameters:addDouble("AL" .. id, "ADX Level", "", Level)
    strategy.parameters:addInteger("DP" .. id, "DMI Period", "", 4)
--~     strategy.parameters:addDouble("DL" .. id, "DMI Level", "", Level)
end
...
--~         AL[i] = instance.parameters:getDouble("AL" .. i)
--~         DL[i] = instance.parameters:getDouble("DL" .. i)
--~         AP[i] = instance.parameters:getInteger("AP" .. i)
        DP[i] = instance.parameters:getInteger("DP" .. i)

        assert(TF[i] ~= "t1", "The time frame must not be tick")

        Source[i] = ExtSubscribe(1 + i, nil, TF[i], instance.parameters.Type == "Bid", "bar")

        Indicator["DMI" .. i] = core.indicators:create("DMI", Source[i], DP[i])
--~         Indicator["ADX" .. i] = core.indicators:create("ADX", Source[i], AP[i])
    end
...
function ExtUpdate(id, source, period) -- The method called every time when a new bid or ask price appears.
    ...
    if ExecutionType == "Live" and id == 1 then
        period = core.findDate(Source[1].close, TickSource:date(period), false)
    end
    ...
    if ExecutionType == "Live" then
        if ONE == Source[1]:serial(period) then
            return
        end
    ...
    local i
    for i = 1, TfNumber, 1 do
--~         Indicator["ADX" .. i]:update(core.UpdateLast)
        Indicator["DMI" .. i]:update(core.UpdateLast)
    end
...
--~     for i = 1, TfNumber, 1 do
--~         if
--~             Indicator["ADX" .. i].DATA:size() <= Indicator["ADX" .. i].DATA:first() + 2 or
--~                 Indicator["DMI" .. i].DATA:size() <= Indicator["DMI" .. i].DATA:first() + 2
--~          then
--~             FLAG = false
--~         end
--~     end
    ...
    if
        core.crossesOver(
            Indicator["DMI" .. 1].DIP,
            Indicator["DMI" .. 1].DIM,
            Indicator["DMI" .. 1].DIP:size() - 2,
            Indicator["DMI" .. 1].DIM:size() - 2
        ) and
            (Indicator["DMI" .. 2].DIP[Indicator["DMI" .. 2].DIP:size() - 2] >
                Indicator["DMI" .. 2].DIM[Indicator["DMI" .. 2].DIM:size() - 2] or
                not ON[2]) and
            (Indicator["DMI" .. 3].DIP[Indicator["DMI" .. 3].DIP:size() - 2] >
                Indicator["DMI" .. 3].DIM[Indicator["DMI" .. 3].DIM:size() - 2] or
                not ON[3]) 
--~             Indicator["ADX" .. 1].DATA[Indicator["ADX" .. 1].DATA:size() - 2] > AL[1] and
--~             (Indicator["ADX" .. 2].DATA[Indicator["ADX" .. 2].DATA:size() - 2] > AL[2] or not ON[2]) and
--~             (Indicator["ADX" .. 3].DATA[Indicator["ADX" .. 3].DATA:size() - 2] > AL[3] or not ON[3])
     then
        if Direction then
            BUY()
        else
