function Init()
    strategy:name("CCI(2)slope")
    strategy:description("CCI(2)slope")
    strategy:type(core.Both)

    strategy.parameters:addGroup("Indicator Parameters")
    strategy.parameters:addString("TimeFrame", "TimeFrame", "TimeFrame", "H8")
...
function Prepare(onlyName)
    ...
    customText = string.format("%s-%s-%d", "FXES_EXTB", "CCI(2)slope", instance.parameters["MagicNumber"])
    instance:name(
        string.format("%s-%s-%d", "CCI(2)slope", instance.bid:instrument(), instance.parameters["MagicNumber"])
    )
...
function ExtUpdate(id, source, period)
    if id == 1 then
        ...
        if allowTrading and HasTrade("B") then
            if CCI.CCI[period] < CCI.CCI[period - 1] then
                core.host:trace("BUY exit condition met. Condition: 'CCI.CCI[period] < CCI.CCI[period - 1]'")
        ...
        if allowTrading and HasTrade("S") then
            if CCI.CCI[period] > CCI.CCI[period - 1] then
                core.host:trace("SELL exit condition met. Condition: 'CCI.CCI[period] > CCI.CCI[period - 1]'")
        ...
        -- BUY ENTRY CONDITION
        if CCI.CCI[period] > CCI.CCI[period - 1] then
            core.host:trace("BUY condition met. Condition: 'CCI.CCI[period] > CCI.CCI[period - 1]'")
        ...
        if CCI.CCI[period] < CCI.CCI[period - 1] then
            core.host:trace("SELL condition met. Condition: 'CCI.CCI[period] < CCI.CCI[period - 1]'")
