function Init()
    strategy:name("CCI(2)zero-lineRegression")
    strategy:description("CCI(2)zero-lineRegression")
    ...
    strategy.parameters:addInteger("CCI Periods", "CCI Periods", "CCI Periods", 2)
    strategy.parameters:addInteger("REGRESSION Periods", "REGRESSION Periods", "REGRESSION Periods", 2)
    strategy.parameters:addInteger("REGRESSION1 Periods", "REGRESSION1 Periods", "REGRESSION1 Periods", 2)
...
function Prepare(onlyName)
    ...
    customText = string.format("%s-%s-%d", "FXES_EXTB", "CCI(2)zero-lineRegression", instance.parameters["MagicNumber"])
    instance:name(
        string.format(
            "%s-%s-%d",
            "CCI(2)zero-lineRegression",
    ...
    CCI = core.indicators:create("CCI", source, instance.parameters["CCI Periods"])
    REGRESSION = core.indicators:create("REGRESSION", source.close, instance.parameters["REGRESSION Periods"])
    REGRESSION1 = core.indicators:create("REGRESSION", source.open, instance.parameters["REGRESSION1 Periods"])
end

function ExtUpdate(id, source, period)
    if id == 1 then
        CCI:update(core.UpdateLast)
        REGRESSION:update(core.UpdateLast)
        REGRESSION1:update(core.UpdateLast)

        core.host:trace("New bar: " .. core.formatDate(source.close:date(period)))
        core.host:trace("CCI >>" .. " CCI:" .. tostring(CCI.CCI[period]))
        core.host:trace("REGRESSION >>" .. " Regression:" .. tostring(REGRESSION.Regression[period]))
        core.host:trace("REGRESSION1 >>" .. " Regression:" .. tostring(REGRESSION1.Regression[period]))
        core.host:trace(
            "Open: " ..
                source.open[period] ..
                    ", High: " ..
        ...
        if allowTrading and HasTrade("B") then
            if CCI.CCI[period] < 0 and REGRESSION.Regression[period] < REGRESSION1.Regression[period] then
                core.host:trace(
                    "BUY exit condition met. Condition: 'CCI.CCI[period] < 0 and REGRESSION.Regression[period] < REGRESSION1.Regression[period]'"
                )
        ...
        if allowTrading and HasTrade("S") then
            if CCI.CCI[period] > 0 and REGRESSION.Regression[period] > REGRESSION1.Regression[period] then
                core.host:trace(
                    "SELL exit condition met. Condition: 'CCI.CCI[period] > 0 and REGRESSION.Regression[period] > REGRESSION1.Regression[period]'"
                )
        ...
        if CCI.CCI[period] > 0 and REGRESSION.Regression[period] > REGRESSION1.Regression[period] then
            core.host:trace(
                "BUY condition met. Condition: 'CCI.CCI[period] > 0 and REGRESSION.Regression[period] > REGRESSION1.Regression[period]'"
            )
        ...
        if CCI.CCI[period] < 0 and REGRESSION.Regression[period] < REGRESSION1.Regression[period] then
            core.host:trace(
                "SELL condition met. Condition: 'CCI.CCI[period] < 0 and REGRESSION.Regression[period] < REGRESSION1.Regression[period]'"
            )
