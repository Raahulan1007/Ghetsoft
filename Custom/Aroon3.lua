function Init()
    strategy:name("Aroon3")
    strategy:description("Aroon3")
    ...
    strategy.parameters:addInteger("AROON Periods", "AROON Periods", "AROON Periods", 3)
...
function Prepare(onlyName)
    ...
    customText = string.format("%s-%s-%d", "FXES_EXTB", "Aroon3", instance.parameters["MagicNumber"])
    instance:name(string.format("%s-%s-%d", "Aroon3", instance.bid:instrument(), instance.parameters["MagicNumber"]))
    ...
    local alertName =
        string.format("%s-%s-%d:", "Aroon3", instance.bid:instrument(), instance.parameters["MagicNumber"])
    ExtSetupSignal(alertName, instance.parameters["ShowAlert"])
    ExtSetupSignalMail(alertName)

    AROON = core.indicators:create("AROON", source, instance.parameters["AROON Periods"])
end
...
function ExtUpdate(id, source, period)
    if id == 1 then
        ...
        core.host:trace("AROON >>" .. " UP:" .. tostring(AROON.UP[period]) .. " DOWN:" .. tostring(AROON.DOWN[period]))
        core.host:trace(
            "Open: " ..
                source.open[period] ..
                    ", High: " ..
        ...
        if allowTrading and HasTrade("B") then
            if AROON.UP[period] < AROON.DOWN[period] then
                core.host:trace("BUY exit condition met. Condition: 'AROON.UP[period] < AROON.DOWN[period]'")
        ...
        if allowTrading and HasTrade("S") then
            if AROON.UP[period] > AROON.DOWN[period] then
                core.host:trace("SELL exit condition met. Condition: 'AROON.UP[period] > AROON.DOWN[period]'")
        ...
        -- BUY ENTRY CONDITION
        if AROON.UP[period] > AROON.DOWN[period] then
            core.host:trace("BUY condition met. Condition: 'AROON.UP[period] > AROON.DOWN[period]'")
        ...
        if AROON.UP[period] < AROON.DOWN[period] then
            core.host:trace("SELL condition met. Condition: 'AROON.UP[period] < AROON.DOWN[period]'")
