function Init()
    strategy:name("DMI(cross)")
    strategy:description("DMI(cross)")
    strategy:type(core.Both)

    strategy.parameters:addGroup("Indicator Parameters")
    strategy.parameters:addString("TimeFrame", "TimeFrame", "TimeFrame", "D1")
    strategy.parameters:setFlag("TimeFrame", core.FLAG_PERIODS)
    strategy.parameters:addInteger("DMI Periods", "DMI Periods", "DMI Periods", 1)
...
function Prepare(onlyName)
    ...
    customText = string.format("%s-%s-%d", "FXES_EXTB", "DMI(cross)", instance.parameters["MagicNumber"])
    instance:name(
        string.format("%s-%s-%d", "DMI(cross)", instance.bid:instrument(), instance.parameters["MagicNumber"])
    )
...
function ExtUpdate(id, source, period)
    if id == 1 then
        ...
        if DMI.DIP[period] > DMI.DIM[period] then
            core.host:trace("BUY condition met. Condition: 'DMI.DIP[period] > DMI.DIM[period]'")
        ...
        if DMI.DIP[period] < DMI.DIM[period] then
            core.host:trace("SELL condition met. Condition: 'DMI.DIP[period] < DMI.DIM[period]'")
