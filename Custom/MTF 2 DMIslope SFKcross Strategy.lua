...
function Init() --The strategy profile initialization
    strategy:name("MTF 2 DMIslope SFKcross Strategy");
    ...
    strategy.parameters:setFlag("TF", core.FLAG_PERIODS);
     ...
     strategy.parameters:addInteger("SFK2 Periods", "SFK2 Periods", "SFK2 Periods", 5); 
     
     strategy.parameters:addInteger("DMI1 Periods", "DMI1 Periods", "DMI1 Periods", 1);
    
      
	
	strategy.parameters:addGroup("1. Time Frame");
	strategy.parameters:addString("TF1", "Time frame", "", "D1");
...
--~ 	Source3 = ExtSubscribe(4, nil, instance.parameters.TF3, instance.parameters.Type == "Bid", "bar");
	...
	DMI1 = core.indicators:create("DMI", Source1, N1);
...
--~ 	or  period3 == -1
	...
	SFK2:update(core.UpdateLast);
	 ...
	 if   
	    DMI1.DIP[period1]>DMI1.DIP[period1-1]  and  core.crossesOver(SFK2.K,SFK2.D,period2)
	 ...
	 then
       ...
       if  
	    ...
	    DMI1.DIP[period1]<DMI1.DIP[period1-1]  and  core.crossesUnder(SFK2.K,SFK2.D,period2)
	...
	if Exit then
		            if     DMI1.DIP[period1]>DMI1.DIP[period1-1]  and  core.crossesOver(SFK2.K,SFK2.D,period2)
					then 			
								if   Direction then
					  ...
					  if   DMI1.DIP[period1]<DMI1.DIP[period1-1]  and  core.crossesUnder(SFK2.K,SFK2.D,period2)
		               then
								   if   Direction then    
										if haveTrades("S") then
										   exitSpecific("S");
