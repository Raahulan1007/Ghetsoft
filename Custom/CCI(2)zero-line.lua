function Init()
    strategy:name("CCI(2)zero-line")
    strategy:description("CCI(2)zero-line")
...
function Prepare(onlyName)
    ...
    customText = string.format("%s-%s-%d", "FXES_EXTB", "CCI(2)zero-line", instance.parameters["MagicNumber"])
    instance:name(
        string.format("%s-%s-%d", "CCI(2)zero-line", instance.bid:instrument(), instance.parameters["MagicNumber"])
    )
...
function ExtUpdate(id, source, period)
    if id == 1 then
        ...
        if allowTrading and HasTrade("B") then
            if CCI.CCI[period] < 0 then
                core.host:trace("BUY exit condition met. Condition: 'CCI.CCI[period] < 0'")
        ...
        if allowTrading and HasTrade("S") then
            if CCI.CCI[period] > 0 then
                core.host:trace("SELL exit condition met. Condition: 'CCI.CCI[period] > 0'")
        ...
        -- BUY ENTRY CONDITION
        if CCI.CCI[period] > 0 then
            core.host:trace("BUY condition met. Condition: 'CCI.CCI[period] > 0'")
        ...
        if CCI.CCI[period] < 0 then
            core.host:trace("SELL condition met. Condition: 'CCI.CCI[period] < 0'")
